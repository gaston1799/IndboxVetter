...
router.post("/google", async (req, res) => {
  try {
    ...
    const p = await verifyIdToken(idToken); // { sub, email, name, picture }
    const user = await User.findOrCreate({ id: p.sub, email: p.email, name: p.name, picture: p.picture });

    req.session.user = { id: user.id, email: user.email, name: user.name, picture: user.picture, role: user.role };
